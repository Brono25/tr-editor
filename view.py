...
class View:
    ...
    def init_ops_frame(self):
        ...
        self.session_label = tk.Label(
            self.ops_frame, textvariable=self.session_label_var
        )
        self.session_label.grid(
            row=0, column=0, columnspan=2
        )  # Centre top: session label
        ...
        self.open_transcript_button.grid(
            row=2, column=0
        )  # Lower middle left: [Open Transcript]
        self.transcript_label_var = tk.StringVar()
        self.transcript_label = tk.Label(
            self.ops_frame, textvariable=self.transcript_label_var
        )
        self.transcript_label.grid(
            row=2, column=1
        )  # Lower middle left: "transcript label"
        ...
        self.open_audiofile_button.grid(
            row=3, column=0
        )  # Bottom left: [Open Audiofile]
        self.audiofile_label_var = tk.StringVar()
        self.audiofile_label = tk.Label(
            self.ops_frame, textvariable=self.audiofile_label_var
        )
    ...
    def init_other_elements(self):
        ...
        data_dump_button = tk.Button(
            self.root, text="Data Dump", command=self.controller.data_dump
        )
